...
(function () {
  ...
  function tickClocks() {
    ...
    clockNodes.forEach(({ tz, cluster, timeEl, statusEl }) => {
      if (!timeEl || !statusEl) return;

      /* Format HH:MM:SS in the target timezone */
      const formatted = timeFormatters.get(tz).format(now);

      /* Derive local hour to compute DAY / NIGHT.
       * hour12:false returns "00"–"23"; parseInt
       * handles the leading zero safely.          */
      const hourStr = hourFormatters.get(tz).format(now);
      const hour = parseInt(hourStr, 10);

      /* Night window: 20:00 – 05:59 local time */
      const isNight = hour >= 20 || hour < 6;
      const status = isNight ? "NIGHT" : "DAY";

      /* Update display text */
      timeEl.textContent = formatted;

      /* Keep datetime attribute machine-readable */
      timeEl.setAttribute("datetime", now.toISOString());

      statusEl.textContent = status;

      /* Drive CSS dark-pivot via class toggle so
       * transitions are handled entirely in CSS. */
      cluster.classList.toggle("is-night", isNight);
    });
  }
